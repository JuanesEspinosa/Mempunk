...
Read the file at "${cfg}" and parse the JSON. Look at "context.includeDirectories" (array of strings). For each directory, check if it contains a CLAUDE.md file. Those are Mempunk vaults.
...
If you loaded a vault via /mempunk earlier in this session, use that vault. Otherwise, read "${cfg}", parse the JSON, and look at "context.includeDirectories" — those are the Mempunk vaults (directories containing CLAUDE.md). If multiple vaults exist and you don't know which was active, ask the user.
...
export const geminiCliAdapter = {
  ...
  getRegisteredDirs() {
    ...
    return cfg.context?.includeDirectories || [];
  },

  addDir(vaultPath) {
    const cfg = readConfig();
    const ctx = cfg.context || {};
    const dirs = ctx.includeDirectories || [];
    if (dirs.includes(vaultPath)) return;
    ctx.includeDirectories = [...dirs, vaultPath];
    // Auto-load GEMINI.md / CLAUDE.md from the vault on /memory reload
    ctx.loadMemoryFromIncludeDirectories = true;
    cfg.context = ctx;
    writeConfig(cfg);
  },
  ...
  removeDir(vaultPath) {
    ...
    const dirs = ctx.includeDirectories || [];
    if (!dirs.includes(vaultPath)) return;
    ctx.includeDirectories = dirs.filter((d) => d !== vaultPath);
    if (ctx.includeDirectories.length === 0) {
      delete ctx.includeDirectories;
      // Drop the companion flag too if no dirs remain
      delete ctx.loadMemoryFromIncludeDirectories;
    }
